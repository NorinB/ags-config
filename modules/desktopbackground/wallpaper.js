...
export default (monitor = 0) => {
  const WALLPAPER_OFFSCREEN_X =
    (WALLPAPER_ZOOM_SCALE - 1) * monitors[monitor].width;
  const WALLPAPER_OFFSCREEN_Y =
    (WALLPAPER_ZOOM_SCALE - 1) * monitors[monitor].height;
  const wallpaperImage = Widget.DrawingArea({
    attribute: {
      ...
      updatePos: (self) => {
        self.setCss(
          `font-size: ${self.attribute.workspace - self.attribute.sideleft + self.attribute.sideright}px;`,
        );
      },
    },
    ...
    setup: (self) => {
      ...
      self
        ...
        .on("draw", (self, cr) => {
          ...
          const workspace = styleContext.get_property(
            "font-size",
          ...
          Gdk.cairo_set_source_pixbuf(
            cr,
            self.attribute.pixbuf,
            -(
              (WALLPAPER_OFFSCREEN_X / (MAX_WORKSPACES + 1)) *
              clamp(workspace, 0, MAX_WORKSPACES + 1)
            ),
        ...
        .hook(
          ...
          (self) => {
            ...
            const scale_x =
              (monitors[monitor].width * WALLPAPER_ZOOM_SCALE) /
              self.attribute.pixbuf.get_width();
            const scale_y =
              (monitors[monitor].height * WALLPAPER_ZOOM_SCALE) /
              self.attribute.pixbuf.get_height();
  ...
  const wallpaperPrompt = Box({
    ...
    children: [
      ...
      Button({
        ...
        onClicked: (self) =>
          Utils.execAsync([SWITCHWALL_SCRIPT_PATH]).catch(print),
      }),
    ],
  });
  const stack = Stack({
